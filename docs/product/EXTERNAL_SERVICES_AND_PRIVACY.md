...
- Unlike Wikimedia Commons/Openverse, Google photos are **not openly licensed** and Google's terms do not permit storing the photo itself long-term. Roadplanner therefore never persists a Google photo URL or its bytes: a saved destination gallery keeps only the durable Google photo resource reference (`photo_name`). Each time the panel actually displays that image, Roadplanner resolves a fresh, short-lived URL from Google on the fly and redirects the browser to it through a signed, time-limited Roadplanner URL (`/api/roadplanner/google_photo/...`, mirroring the existing OneDrive personal-media redirect) - the resolved URL and image bytes are never written to disk. Because of this fresh-resolve step, Google photo requests are also excluded from the normal 12-hour destination-image search cache.
- Google Places Photo requests are a separately billed Google Cloud SKU, independent of the Places Text Search budget above; review Google's current pricing before enabling this for regular use. Displaying a saved Google photo spends one photo-quota request on every view (each redirect re-resolves against Google), not just once at save time - keep the daily photo limit in mind if a gallery with Google photos is opened frequently.
