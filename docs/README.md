# Roadplanner documentation

This directory contains the canonical project documentation. Root-level files remain short entry points for GitHub visitors; detailed contracts and procedures live here.

## Architecture

- [Architecture overview](../ARCHITECTURE.md)
- [Architecture documentation index](architecture/README.md)
- [Roadbook v1](architecture/ROADBOOK_V1.md)
- [Plugin API v1](architecture/PLUGIN_API_V1.md)
- [Architecture Decision Records](architecture/adr/README.md)

## Development

- [Development index](development/README.md)
- [Repository structure](development/REPOSITORY_STRUCTURE.md)
- [Development workflow](development/DEVELOPMENT_WORKFLOW.md)
- [Patch workflow](development/PATCH_WORKFLOW.md)
- [Branching model](development/BRANCHING.md)
- [Commit conventions](development/COMMIT_CONVENTIONS.md)
- [Definition of Done](development/DEFINITION_OF_DONE.md)
- [Test strategy](development/TEST_STRATEGY.md)
- [Release process](development/RELEASE_PROCESS.md)
- [Release checklist](development/RELEASE_CHECKLIST.md)
- [HACS setup](development/HACS_SETUP.md)
- [Public-source checklist](development/PUBLICATION_CHECKLIST.md)
- [iPad and Codespaces workflow](development/IPAD_CODESPACES_WORKFLOW.md)
- [License policy](development/LICENSE_POLICY.md)
- [Task template](development/TASK_TEMPLATE.md)

## Planning

- [Roadmap](../ROADMAP.md)
- [Backlog](../BACKLOG.md)
- [Changelog](../CHANGELOG.md)

## Historical material

Historical release documents are retained under [`docs/legacy/`](legacy/README.md). They describe the imported baseline and are not the current architecture contract.
