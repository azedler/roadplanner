...
export const tripDayStopMixin = {
      ...
      <section class="route-layout">
        ...
        <aside class="day-facts panel-card">
          ...
          <div class="button-row">
            ...
            ${this._canEdit() ? `<button class="secondary-button" type="button" data-action="edit-day" data-day-id="${escapeHtml(day.id)}"><ha-icon icon="mdi:pencil-outline"></ha-icon> Tag bearbeiten</button><button class="secondary-button" type="button" data-action="add-stop" data-day-id="${escapeHtml(day.id)}"><ha-icon icon="mdi:map-marker-plus-outline"></ha-icon> Stopp</button><button class="secondary-button" type="button" data-action="add-day"><ha-icon icon="mdi:calendar-plus"></ha-icon> Reisetag</button>${this._renderDayOrderButtons(day)}<button class="text-button danger-text" type="button" data-action="delete-day" data-day-id="${escapeHtml(day.id)}"><ha-icon icon="mdi:trash-can-outline"></ha-icon> Tag löschen</button>` : ""}
          </div>
        </aside>
      </section>
  ...
  async _runStopFormP4nLookup(form) {
    if (!form) return;
    const values = Object.fromEntries(new FormData(form).entries());
    const reference = park4nightReference(values.name, values.notes, values.address);
    if (!reference) {
      this._showToast("Kein Park4Night-Verweis gefunden - füge z. B. „p4n 506374“ oder den park4night.com-Link in Name oder Notizen ein.", "error", 6500);
      return;
    }
    const result = await this._runAction("park4night_lookup", {
      text: [values.name, values.notes, values.address].filter(Boolean).join("\n"),
    }, "", { refresh: false, errorTitle: "Park4Night-Seite konnte nicht gelesen werden" });
    const facts = result?.result;
    if (!facts) return;
    // Prefill only - the user reviews the values and saving the form is the
    // confirmation. AI-read data never reaches the roadbook on its own.
    const setField = (fieldName, value, { overwrite = false } = {}) => {
      const input = form.querySelector(`[name='${fieldName}']`);
      if (!input || value == null || value === "") return;
      if (!overwrite && cleanText(input.value)) return;
      input.value = String(value);
    };
    setField("latitude", facts.latitude, { overwrite: true });
    ...
    const summary = [facts.price_text, facts.rating_text ? `Bewertung ${facts.rating_text}` : "", facts.summary].filter(Boolean).join(" · ");
    this._showToast(`Von der Park4Night-Seite übernommen${summary ? ` (${summary})` : ""} - bitte prüfen und speichern.`, "success", 6500);
  },
