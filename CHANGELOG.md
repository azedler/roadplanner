...
- **Siebzehn bezahlte Knöpfe sahen gratis aus.** Der lesende Live-Durchlauf fand rund ein Dutzend Aktionen mit echten Gemini-Aufrufen ohne Kosten-Badge — darunter „Systemcheck", der nach Diagnose klingt und einen echten Testaufruf enthält, und „Als PDF", dessen Personen-Zusammenfassungen von Gemini kommen (den fand erst der neue Test). Alle sind jetzt in der Kostentabelle deklariert und werden über den gemeinsamen `actionButton` gezeichnet: Modell-Symbol, Kostenhinweis im Tooltip, einheitliche Optik. Ein neuer Contract-Test liest den Panel-Dispatcher und schlägt fehl, sobald eine Aktion einen bezahlten Provider-Einstiegspunkt erreicht, ohne als `COST_MODEL` deklariert zu sein — eine künftige bezahlte Funktion bekommt ihr Badge damit, bevor sie mergen kann.
  ...
  Die adversariale Prüfung fand danach vier weitere Knöpfe, die sich hinter einem *anderen* DOM-Namen versteckt hatten („Stopps anreichern", „Park4Night-Daten lesen", die Videoanalyse, der Sendeknopf im Reisegespräch) — ihre Kosten stehen unter dem Namen der Aktion, die sie auslösen, und eine Alias-Tabelle verbindet beides; auch sie liest der Test. Zwei weitere bezahlte Aktionen (Dokumentanalyse, universeller Import) waren noch gar nicht deklariert. Und zwei Einträge waren **falsch herum**: die GPS-Vervollständigung baut ihre Vorschläge lokal und löst sie über OpenStreetMap auf — sie kostet nichts und wird nicht mehr als bezahlt markiert. Die Hinweise zu Park4Night und „Link auslesen" sagen jetzt außerdem, dass die Seiten zuerst *direkt* gelesen werden und Gemini nur einspringt, wenn das misslingt.
...
- **„Film erstellen" mit KI-Musik stürzte ab — und „Musik auflegen" war seit einer Woche kaputt.** Beim A/B/C-Umbau hatte eine unachtsame Ersetzung an **zwei** Stellen eine *Funktion* als Datenargument in den Paketbau geschoben (`'function' object is not iterable`). Betroffen war nicht nur der neue Renderweg, sondern auch das Auflegen des vollen Soundtracks auf einen fertigen Film — jeder Versuch seit 4.113.0 scheiterte mit diesem Fehler.
  ...
  **„Mit KI-Musik" heißt jetzt: stumm rendern, danach automatisch auflegen.** Sobald der Render fertig ist, legt das Panel die Musik auf — angepasst an die gemessene Länge, mit kopiertem Videostream, in Sekunden. Fehlt die Musik noch, fragt „Film erstellen" zuerst mit Preis und Aufrufzahl; erst nach der Zustimmung wird erzeugt und dann gerendert. Ein Abbruch startet nichts. Wird das Panel während des Renders geschlossen, bleibt am fertigen Film der Knopf „Musik auflegen".
  ...
  Eine adversariale Prüfung des Fixes fand drei tiefere Fehler, alle behoben: **Das Auflegen suchte die Abschnitte falsch** — ohne Szenenplan und mit der gemessenen statt der geschätzten Länge, während Angebot und Erzeugung die Cache-Schlüssel aus Schätzung plus Plan bildeten. Gemini-geplante Abschnitte hätten *nie* gefunden werden können; bezahlte Musik wäre dauerhaft als „nie erzeugt" gemeldet worden. Der Mux fragt jetzt exakt so wie das Angebot und nutzt die gemessene Länge nur zum Einpassen. **Die Auto-Mux-Absicht** ist an ihren Auftrag und ihre Reise gebunden statt an „was auch immer als Nächstes fertig wird" — ein Reisewechsel während des Renders vertont nicht mehr die falsche Reise. Und **ein Reisewechsel** räumt jetzt den Filmblock auf: Das Musikangebot einer Reise kann den Render der nächsten weder freischalten noch bepreisen.
...
- **Die Musikabschnitte wurden immer noch nach Division gesetzt, nicht nach dem Film.** Das Cue Sheet, der Auftrag an das Modell und die Prüfung seiner Antwort waren gebaut und hatten grüne Tests — und liefen nie. Kein Aufrufer übergab den Szenenplan, also brach der Planer ab, bevor er irgendetwas tat. Auf dem echten Panel war es zu sehen, in einer Zeile, die es seit gestern gibt: „Abschnitte geplant von — der Verteilung im Film". Die Abschnittsnamen sagten es ebenfalls: „Norden und große Reise **1**" und „**2**" ist der arithmetische Teiler, der die Hälften eines zu lang geratenen Abschnitts durchnummeriert, kein Modell, das eine Reise gelesen hat.
  ...
  Der Plan war die ganze Zeit da. Die Längenschätzung fährt den echten Szenenplaner, nahm die Bildzahl ab und warf den Plan weg — obwohl er nichts kostet, weil der Planer Bilder zählt und keins öffnet. **Angebot, Erzeugung und Render nehmen Länge und Form jetzt aus demselben einen Lauf.** Das ist mehr als Ordnung: Ein Preis, den jemand akzeptiert, muss genau die Abschnitte kaufen, die im Dialog standen.
...
- **Eine Ablehnung ist eine Antwort und darf nicht zweimal bezahlt werden.** Von der Karte des echten Systems, an eigenen Urlaubsaufnahmen: *„20260719_135620000_iOS.MOV: Gemini hat die Anfrage aus Sicherheitsgründen blockiert (PROHIBITED_CONTENT)"*. Eine solche Ablehnung ist **endgültig**: dieselben Sekunden derselben Aufnahme, beurteilt vom selben Modell nach derselben Richtlinie, werden wieder abgelehnt. Sie galt aber als Fehlschlag, wurde deshalb nie gespeichert — und jeder künftige Lauf plante sie erneut, lud die Aufnahme erneut herunter, schnitt erneut einen Ausschnitt und fragte erneut. Die Karte bot „1 neue Analyse" für ein Fenster an, das nur abgelehnt werden kann. Diese Filter weisen ganz gewöhnliche Familienaufnahmen regelmäßig und zu Unrecht zurück; das ist also kein seltener Sonderfall. Sie wird jetzt so abgelegt, wie eine unlesbare Antwort schon immer abgelegt wurde: kein Moment hier, samt Begründung. Jeder **andere** Providerfehler bleibt ein Fehlschlag — eine abgerissene Verbindung kann beim nächsten Mal klappen.
  ...
  Und es wird ausgesprochen, statt unter „analysiert" zu verschwinden: **Abgelehnt** ist ein eigenes Ergebnis auf der Karte, mit dem Satz, den man dazu braucht — die KI hat sich die Aufnahme nicht angesehen, sie wird nicht erneut gefragt, es wird keinen Clip geben, und die Fotos dieses Tages sind unberührt. Ein Clip, der einfach nie im Film auftaucht und für den es keine Erklärung gibt, wäre dasselbe Muster wie eine fehlende Antwort, die als Zustand dargestellt wird.
...
- **Eine nicht ladbare Aufnahme wurde für jedes ihrer Fenster erneut geladen.** Beim Schreiben des Tests aufgefallen und vermutlich der eigentliche Grund, warum zehn Minuten nichts ergaben: Eine zweiminütige Aufnahme wird als **drei Fenster** angeboten, und gemerkt wurden nur die *erfolgreichen* Downloads. Eine Datei, die nicht kam, wurde also dreimal geholt — bei einem Download, der hängt statt abzulehnen, ist das die Stillstandsgrenze dreimal hintereinander für eine einzige unbrauchbare Datei, und ein Lauf, der festzustecken scheint, während er sich geduldig wiederholt. Ein gescheiterter Download wird jetzt für die Dauer des Laufs gemerkt: Die restlichen Fenster derselben Aufnahme scheitern sofort, mit der Begründung, die der Download geliefert hat.
...
- **Diese Korrektur wäre nie angekommen.** Der Story-Kontext-Hash — der Cache-Schlüssel der gespeicherten Regie — kannte je Stopp nur `stop_id`, `name` und `kind`. Eine bessere Klassifikation bewegt keines der drei, also hätte der zwischengespeicherte Titel jede Verbesserung überlebt, dauerhaft und kostenlos. Der Hash sieht jetzt, **was ein Stopp bedeutet**: Genau die davon abhängigen Regie-Fassungen sind veraltet, keine andere — kein globales Cache-Löschen und kein neuer Gemini-Aufruf, bis der Nutzer die Redaktion selbst startet.

- **Dasselbe Foto konnte in zwei Kapiteln landen.** Ein Foto, das an Tag A hängt, dessen Stopp aber auch in Tag B vorkommt, wurde von **beiden** eingesammelt — ein Bild zweimal im Film, ohne jede Meldung. Der verlinkte Tag gewinnt (das ist die Zuordnung, die jemand getroffen hat); ein Anspruchsvermerk fängt den Restfall ab, ein Foto ganz ohne Tag, gefunden über einen Stopp, den sich zwei Tage teilen.

- **Zwei Bilder einer Serie standen direkt nebeneinander.** Das Serienlimit entschied *wie viele*, nie *wo*. `spread_series()` ist eine **Umsortierung und nie eine Auswahl** — jede Kennung geht rein, jede kommt raus — und schiebt eine wiederholte Gruppe hinter das nächste andere Bild. Ein Tag, der wirklich eine einzige lange Serie ist, behält seine Reihenfolge. Deterministisch, wie der Renderer es verlangt.

### Changed

- **200 gegen 197 Bilder: nachverfolgt, und die Anzeige korrigiert.** Die Zuteilung vergibt 200. Beim Bau des Filmpakets wird jedes Foto geholt und verkleinert; Fehlschläge zählen als `unavailable_media` und landen nicht im Paket. Das Outro summiert, was **tatsächlich im Paket liegt** — 197 stimmt also als *gerendert*, und drei Fotos waren nicht abrufbar oder nicht dekodierbar. Zwei verschiedene Größen, also sagen die Bezeichnungen jetzt welche: Die Vorschau nennt „200 Bilder **ausgewählt**".
...
- **Das Add-on-Image enthielt ein Modul nicht, das der Renderer importiert.** Der Dockerfile zählt die Laufzeitquellen einzeln auf, und das scheiterte auf die unangenehmste denkbare Art: Image gebaut, Container gestartet, Heartbeat meldet „bereit", korrekte Version — und erst der **erste Render** stirbt an der fehlenden Datei. Alles, was Gesundheit meldet, sagte, dem Renderer gehe es gut. Ein Test folgt jetzt den Importen ab dem Einstiegspunkt und vergleicht sie mit der Liste im Dockerfile.

**Das Add-on muss mit aktualisiert werden** (Version 0.20.0). Ohne das neue Image bleibt der Abbruch bestehen, egal welche Integrationsversion läuft.
...
- **Ein aufgebrauchtes Tageslimit wurde als Erfolg gemeldet und lief vierzig Runden ins Leere.** Ist das Budget weg, ist das eine Tatsache über die ganze Reise, nicht über einen Tag: Der Lauf marschierte trotzdem weiter, stempelte „Tageslimit erreicht" über den Hinweis jedes verbleibenden Tages — und überschrieb damit die nützlichere Auskunft, die dort stand — und meldete dem Panel ein `remaining`, auf dem dessen Schleife vierzig folgenlose Runden drehte (daher der Lauf, der „in 5 Sekunden fertig" war). Jetzt hält der Lauf an, lässt die übrigen Tage unangetastet, und das Panel sagt klar, dass das Tageslimit erreicht ist und morgen erneut gedrückt werden soll — statt „273 Bilder gewählt" zu melden. Der Hinweis nennt jetzt auch die Zahlen (`60 von 60 Aufrufen heute`).
...
- **Der Fehlertext beim Camper-Bild nannte die falsche Ursache.** Jede Ablehnung sagte „Erwartet wird ein PNG mit transparentem Hintergrund" — auch dann, wenn das Bild genau das war und lediglich über der Größengrenze landete. Jemandem zu sagen, er solle etwas reparieren, das nicht kaputt ist, ist schlimmer als nichts zu sagen: Er ändert die eine Sache, die nicht helfen kann, und schließt daraus, dass die Funktion nicht funktioniert.
  ...
  Jetzt gibt es drei verschiedene Antworten, und nur eine davon handelt von Transparenz: **nicht lesbar** (iPhone-HEIC wird namentlich genannt, weil das der wahrscheinlichste Fall am Handy ist), **kein transparenter Hintergrund** (das Bild würde als Rechteck auf der Karte liegen) und **zu groß**.

- **Eine detaillierte Zeichnung wird verkleinert statt abgelehnt.** Über der Größengrenze war bisher ein Grund, jemanden wegzuschicken. Die Figur wird höchstens auf einem Drittel eines 1280-Pixel-Bildes gezeigt — den Unterschied zwischen 16 Millionen Farben und 256 sieht dort niemand. Also wird in Stufen reduziert (volle Farbtiefe → 256 Farben → 128 → 128 und kleiner), und erst wenn auch das nicht reicht, gibt es eine Absage.
