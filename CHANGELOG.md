...
- Internal: extracted the ChangeSet glue (`preview_changeset`, `inspect_changeset_for_import`, `apply_changeset`, `adopt_external_changes`) out of `RoadplannerStore` into a dedicated `changeset_operations.py` collaborator, and resolved `changeset.py`'s only circular-dependency risk with `roadplanner.py` by having it import directly from the leaf modules that now own those symbols (eleventh step of the `roadplanner.py` decomposition). No functional or behavior changes.
- Internal: extracted the bounded handoff-context JSON/Markdown generation (`get_context_payload`, `get_context_markdown`, `write_context`) out of `RoadplannerStore` into a dedicated `context_export.py` collaborator, completing the `roadplanner.py` decomposition (3605 → 403 lines across 12 focused modules). No functional or behavior changes.
